...
{
if(IsPlayerInAnyVehicle(playerid))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voc� est� em um veiculo");
if(GetPlayerSpecialAction(playerid) == SPECIAL_ACTION_USEJETPACK)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce j� est� usando um jetpack");
...
CMD:v(playerid)
{
if(IsPlayerInAnyVehicle(playerid))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce j� est� em um veiculo!");
...
{
new VehicleName[64];
if(sscanf(params, "s", VehicleName))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Digite: /vm [nome do veiculo]");
CriarVeiculoParaPlayer(playerid, GetVehicleModelIDFromName(VehicleName));
return 1;
}*/
...
{
new PlayerIDPM, string[256], string2[256], Mensagem[256];
if(sscanf(params, "is", PlayerIDPM, Mensagem))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Digite: /pm [id] [mensagem]");
if(PlayerIDPM == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce n�o pode mandar uma mensagem para voc� mesmo");
if(!IsPlayerConnected(PlayerIDPM))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player invalido!");
format(string,sizeof(string), "| PM | Mensagem recebida de %s[%d] ( %s )",GetPlayerNameEx(playerid),playerid,Mensagem);
format(string2,sizeof(string2), "| PM | Mensagem enviada para o %s[%d] ( %s )",GetPlayerNameEx(PlayerIDPM),PlayerIDPM,Mensagem);
PlayerPlaySound(PlayerIDPM,1057,0.0,0.0,0.0);
GameTextForPlayer(PlayerIDPM, "~b~~h~~h~~h~PM RECEBIDA", 3000, 1);
SendClientMessage(PlayerIDPM,COR_PM,string);
...
else return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voc� n�o tem permiss�o!");
return 1;
}
...
else return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce n�o tem permiss�o!");
return 1;
}
...
if(PlayerIDKickado == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce n�o pode kickar voc� mesmo");
...
if(PlayerIDBanido == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce n�o pode banir voc� mesmo");
...
{
if(GetPlayerAdminLevel(playerid) != 3)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce n�o tem permiss�o");
...
if(GetPlayerAdminLevel(ID) == levels)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player j� possui esse level de permiss�o");
if(levels == 0){
format(string3,sizeof(string3),"| ADMIN | O(A) Administrador(a) %s removeu sua permiss�o de Admin!",GetPlayerNameEx(playerid));
format(string4,sizeof(string4),"| INFO | Voc� removeu a permiss�o de Admin de %s",GetPlayerNameEx(ID));
SendClientMessage(ID,COR_ADM,string3);
SendClientMessage(playerid,-1,string4);
SetPlayerAdminLevel(ID,levels);}else{
format(string,sizeof(string),"| AMDIN | O(A) Administrador(a) %s deu level %d de Admin para voc�!",GetPlayerNameEx(playerid),levels);
format(string2,sizeof(string2),"| INFO | Voc� deu level %d de Admin para %s",levels,GetPlayerNameEx(ID));
...
{
if(!IsPlayerAdmin(playerid))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voc� n�o tem permiss�o!");
...
{
if(GetPlayerAdminLevel(playerid) == 3){
SendClientMessageToAll(-1, "| SERVER | {15ff00}O server est� reiniciando!");
SendRconCommand("gmx");}
else return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voc� n�o tem permiss�o!");
return 1;
}
...
if(PlayerIDFreeze == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voc� n�o pode congelar voc� mesmo!");
if(Freeze[PlayerIDFreeze] == true)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player ja est� congelado!");
if(!IsPlayerConnected(PlayerIDFreeze))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player invalido");
SendClientMessage(playerid, -1,"| INFO | {15ff00}Voc� congelou um player!");
...
if(Freeze[PlayerIDFreeze] == false)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player n�o est� congelado!");
if(!IsPlayerConnected(PlayerIDFreeze))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player invalido!");
...
if(!IsPlayerConnected(PlayerID))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player inv�lido!");
...
if(!IsPlayerConnected(PlayerID))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player inv�lido");
format(string,sizeof(string), "| ADMIN | O(A) Administrador(a) %s deu {12d800}$%d {168fad}para voc�!",GetPlayerNameEx(playerid),Quantidade);
...
{
if(!success){
SendClientMessage(playerid,COR_ERRO,"| ERRO | Comando inv�lido");}
return 1;
}
...
if(dialogid == DialogWeapons)
{
  if(response)
  {
	...
	if(listitem == 7)
	{
	ShowPlayerDialog(playerid,DialogBrancas,DIALOG_STYLE_LIST,"Armas Brancas","1\tSoco Ingl�s\n2\tTaco de Golf\n3\tCassetete\n4\tFaca\n5\tTaco de Baseball\n6\tP�\n7\tTaco de Bilhar\n8\tKatana\n9\tMoto Serra\n10\tDildo\n11\tDildo 2\n12\tVibrador\n13\tVibrador 2\n14\tFlores\n15\tBengala","Selecionar","Voltar");
	}
	if(listitem == 8)
	{
...
if(dialogid == DialogVehicle)
{
  if(response)
  {
	...
	ShowPlayerDialog(playerid,Servi�os_Publicos,DIALOG_STYLE_LIST,"{ff0000} Servi�os Publicos",allvehicles,"Selecionar","Voltar");
	}
	if(listitem == 8)
	{
...
format(string, 128, "{ffffff}| CONTA |{15ff00} %s Sua conta foi registrada com sucesso. Sua senha �{ffffff} %s", GetPlayerNameEx(playerid), inputtext);
...
SendClientMessage(playerid, -1, "| INFO | Voc� possui permiss�o level 3 de Admin!");
}else{Kick(playerid);}
return 1;
}
