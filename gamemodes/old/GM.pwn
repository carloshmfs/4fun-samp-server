...
CMD:jp(playerid)
{
if(IsPlayerInAnyVehicle(playerid))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce está em um veiculo");
...
return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce já está em um veiculo!");}
else{
...
if(sscanf(params, "us", PlayerID, Mensagem))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Uso: /pm [playerid] [mensagem]");
if(PlayerID == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce não pode mandar uma mensagem para si mesmo");
...
if(sscanf(params, "i", WeatherID))return SendClientMessage( playerid, COR_ERRO, "| ERRO | Uso: /tempo [weatherid]" );
...
if(sscanf(params, "is", PlayerIDKickado, Motivo))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Uso: /kick [playerid] [motivo]");
if(PlayerIDKickado == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce não pode kickar voce mesmo");
...
if(sscanf(params, "is", PlayerIDBanido, Motivo))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Uso: /ban [playerid] [motivo]");
if(PlayerIDBanido == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce não pode banir voce mesmo");
...
{
if(PlayerInfo[playerid][pAdmin] != 3)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Voce não tem permissão");
new ID,levels,string[256],string2[256],string3[256],string4[256];
if(sscanf(params,"ui",ID,levels))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Uso: /setadmin [playerid] [level]");
...
if(PlayerInfo[ID][pAdmin] == levels)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player já possui esse level de permissão");
if(levels == 0){
...
format(string2,sizeof(string2),"| INFO | {15ff00}Você deu level %d de Admin para %s",levels,GetPlayerNameEx(ID));
...
if(sscanf(params, "i", PlayerIDFreeze))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Digite: /congelar [id]");
if(PlayerIDFreeze == playerid)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Você não pode congelar você mesmo!");
if(Freeze[PlayerIDFreeze] == 1)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player ja está congelado!");
...
if(PlayerInfo[playerid][pAdmin] >= 2){
new PlayerIDFreeze, string[256];
if(sscanf(params, "i", PlayerIDFreeze))return SendClientMessage(playerid, COR_ERRO, "| ERRO | Digite: /descongelar [id]");
if(Freeze[PlayerIDFreeze] == 0)return SendClientMessage(playerid, COR_ERRO, "| ERRO | Este player não está congelado!");
if(!IsPlayerConnected(PlayerIDFreeze))return SendClientMessage(playerid, COR_ERRO,"| ERRO | Player invalido!");
...
format(file, sizeof(file), PATH, GetPlayerNameEx(playerid));
if(!response) return Kick(playerid);
if(!strlen(inputtext))return ShowPlayerDialog(playerid, Login2, DIALOG_STYLE_PASSWORD, "{ff0000}  Conta", "{B9C9BF} Bem-vindo!\n\n {ffffff}Status da Conta: {12d800}Registrado\n\n {ffffff}Entre com sua senha abaixo:", "Login", "Sair");
...
{
new pos[128];
format(pos, sizeof(pos), "| INFO | {15ff00}Waypoint localizado em  {B9C9BF}X: %f  Y: %f  Z: %f", fX, fY, fZ);
